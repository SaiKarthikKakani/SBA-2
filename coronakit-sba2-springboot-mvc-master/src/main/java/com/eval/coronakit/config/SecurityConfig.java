...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/ADMIN/**").hasRole("ADMIN").antMatchers("USER/**").hasRole("USER").and()
				.formLogin().loginPage("/custom-login").loginProcessingUrl("/validate").defaultSuccessUrl("/home")
				.permitAll().and()
				.logout().permitAll().and().exceptionHandling().accessDeniedPage("/custom-error");
	}
}
